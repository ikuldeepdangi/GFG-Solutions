...
class Solution {
    static String matrixChainOrder(int p[], int n) {
        int[][] dp = new int[n][n]; // to store the minimum number of multiplications
        int[][] s = new int[n][n]; // to store the split index for optimal parenthesization
        ...
        return printParenthesis(1, n - 1, s);
    }

    static String printParenthesis(int i, int j, int[][] s) {
        if (i == j) {
            return String.valueOf((char) ('A' + i - 1));
        } else {
            return "(" + printParenthesis(i, s[i][j], s) + printParenthesis(s[i][j] + 1, j, s) + ")";
        }
    }
}
